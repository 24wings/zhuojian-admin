...
export class AppFruitService {
  ...
  get fruitProductFields(): Field[] {
    return [
      ...
      // { label: '简介', key: 'summar/y', value: '', validators: Validators.required, controlType: 'text' },
  ...
  async listFruitUsers(page?: number, pageSize?: number) {
    return this.isMocking
      ? this.mock.listFruitUsers()
      : this.api.Get(this.userListApi + "?adminId=" + this.admin.admin._id);
  }
  ...
  async deleteFruitUser(userId: string) {
    this.api.Delete(
      this.deleteFruitUserApi + `?adminId=${this.adminId}&userId=${userId}`
    );
  }
  async searchUserListByKeyword(keyword: string) {
    return this.api.Get(
      this.searchUserList + `?keyword=${keyword}&adminId=${this.adminId}`
    );
  }

  updateFruitUser(userId: string, user: FruitUser) {
    return this.api.Put(
      this.updateFruitUserApi + `?userId=${userId}&adminId=${this.adminId}`,
      user
    );
  }
  ...
  object2FruitProductGroupFieldsValue(obj: Object) {
    let result = {};
    this.fruitProductGroupFields.forEach(
      field => (result[field.key] = obj[field.key])
    );
    return <any>result;
  }
  ...
  createProductGroup(productGroup: FruitProductGroup) {
    return this.api.Post(
      this.createProductGroupApi + `?adminId=${this.adminId}`,
      productGroup
    );
  }
  ...
  updateProductGroup(groupId: string, productGroup: FruitProductGroup) {
    return this.api.Put(
      this.updateProductGroupApi +
        `?adminId=${this.adminId}&groupId=${groupId}`,
      productGroup
    );
  }
  ...
  deleteProductGroup(groupId: string) {
    return this.api.Delete(
      this.deleteProductGroupApi + `?adminId=${this.adminId}&groupId=${groupId}`
    );
  }

  getProductGroupProducts(groupId: string) {
    return this.api.Get(
      this.productGroupProductsApi +
        `?adminId=${this.adminId}&groupId=${groupId}`
    );
  }
  ...
  createProduct(groupId: string, newProduct: FruitProduct) {
    ...
    return this.api.Post(
      this.createProductApi + `?adminId=${this.adminId}&groupId=${groupId}`,
      newProduct
    );
  }
  ...
  deleteProduct(productId: string) {
    return this.api.Delete(
      this.deleteProductApi + `?productId=${productId}&admin=${this.adminId}`
    );
  }

  updateProduct(productId: string, newProduct) {
    return this.api.Put(
      this.updateProductApi + `?productId=${productId}&adminId=${this.adminId}`,
      newProduct
    );
  }
  ...
  getAdminInfo() {
    return this.isMocking
      ? this.mock.admin
      : this.api.Get(this.getAdminInfoApi);
  }
  adminLogout() {
  ...
  getTodayOrders(orderState: number) {
    return this.isMocking
      ? this.mock.getTodayOrders()
      : this.api.Get(
          this.getTodayOrdersApi +
            (orderState == null || orderState == undefined
              ? ""
  ...
  getMainInfo() {
    return this.isMocking
      ? this.mock.getMainInfo()
      : this.api.Get(this.getMainInfoApi);
  }
  getUserOrder() {
    return [
      {
        ...
        product: { name: "边瓜" },
        num: 40,
        orderUser: { nickname: "用户甲" },
        state: FruitOrderState.UnConfirm,
        admin: "",
        address: "杭州西湖区古荔湾"
      },
      {
        ...
        num: 40,
        orderUser: { nickname: "用户甲" },
        state: FruitOrderState.SendProduct,
        admin: "",
        address: "杭州西湖区古荔湾"
      },
      {
        ...
        num: 40,
        orderUser: { nickname: "用户甲" },
        state: FruitOrderState.ReciveProduct,
        admin: "",
        address: "杭州西湖区古荔湾"
      },
      {
